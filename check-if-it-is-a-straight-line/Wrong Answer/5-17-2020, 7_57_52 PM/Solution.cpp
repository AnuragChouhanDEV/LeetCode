...
public:
    bool checkStraightLine(vector<vector<int>>& coordinates) {
        ...
        for (int i = 1; i < size; ++i) {
            //cout << coordinates[i][0]-coordinates[i-1][0] << " " << coordinates[i][1]-coordinates[i-1][1] << endl;
            if ((x_diff != abs(coordinates[i][0]-coordinates[i-1][0])) || (y_diff != abs(coordinates[i][1]-coordinates[i-1][1])))
                return false;
        }
        return true;
    }
